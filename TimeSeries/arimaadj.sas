...
IMPROVEMENTS:	(2011/09/17)
				...
				take into account the seasonal part to:
					- Analyze if seasonal differences are needed (this can be done still
					with the Dickey-Fuller test in the IDENTIFY statement by using a period
					parameter in the STATIONARY option (which can be at most 12 however!)
					NOTE however that when a differencing is applied to the original series
					also a SEASONAL difference is applied, which is suggested as usual
					practice in the literature.
					- Include the seasonal part in the model! (as an ARMA(P,D,Q) part)
				In addition, the identification part of the ARMA(p,q) model is done WITHOUT
				taking seasonality into consideration. This will most likely give very high
				values for the p and q orders when seasonality is present. I think one way
				of including the seasonality in this latter analysis is as follows:
					- Apply the following seasonal models to the de-trended or differenced
					time series: (P=1,Q=0), (P=0,Q=1), (P=1,Q=1). These are the only models
					that should be tried because usually seasonality has very low order of
					P and Q (specially when the time series is not so long as to include
					many seasonal periods)
					- For EACH of the above models, apply the ESACF method to the
					de-seasonalized series, which I think can be obtained by substracting
					the original series to the forecast/predictions generated by the purely
					seasonal model being considered.
					- Try all ARMA(p,q)xARMA(P,Q) models to the de-trended series, for each
					of the 3 considered seasonal (P,Q) combinations.
					- Finally choose the best model over all those tried giving smallest MAPE
					on the validation sample.
				(This problem was seen with the very simple Airline data available at SASHELP.AIR)

				(2011/09/23)
				- ESTIMATE TRANSFER FUNCTION: Currently there is no estimation of the transfer
				function between the target variable and the input variables.
				In order to do this estimation, all input variables should be first prewhitened
				and then an estimation of the transfer function should be carried out (how to do
				so automatically? --well see documentation of HPFDIAGNOSE for ideas under Details
				->  Transfer Functions in an ARIMAX Model)
				In order to prewhiten the input series, do an IDENTIFY statement for each input
				variable as in the following example:
					PROC ARIMA data=tofit;
						IDENFITY var=x;
						ESTIMATE p=1 q=2;		* These orders could be high enough (e.g. p=3, q=3) in order to avoid identifying the series x!!;
						IDENTIFY var=y crosscorr=x;	* The cross-correlation function is Cor(y(t),x(t-s)) => causal models of x->y should have peaks on the right of the graph (positive lags);
					RUN;
				This call to PROC ARIMA first pre-whitenes the x series (assuming that the
				ARMA(1,2) model is the correct model) and then computes the cross correlation
				between the pre-whitened y series and the pre-whitened x series in the following
				IDENTIFY step.
				Note that the CLEAR option can be used in the IDENTIFY statement to remove
				any pre-whitening of input and target series done in previous IDENTIFY/ESTIMATE
				steps.
				*********************************** WARNING ************************************
				Note the following important point stated in the documentation about multiple
				inputs:
				<<
				When identifying a transfer function model with multiple input variables, the
				cross-correlation functions can be misleading if the input series are correlated
				with each other. Any dependencies among two or more input series will confound
				their cross-correlations with the response series.
				>>
				So it seems that the identification of transfer function models when more than
				one variable is present is really difficult!!!
				*********************************** WARNING ************************************
				PROBLEM TO SOLVE: Next comes the issue on how the actual transfer function model
				to entertain is estimated using the cross-correlation information! I don't have
				any method for that at this point... (Wei says to do it manually --pag.298)

				(2012/05/31)
				- STATIONARITY TESTS: Iterate on the stationarity analysis process so that a
				differencing larger than 1 is allowed. This means, perform the Dickey-Fuller
				stationarity test once more after the first analysis (in case the original series
				did not turn out stationary) to check if the resulting new series is stationary.
				If it is not, repeat the process until a stationary series is obtained.

				- FORECASTS: Extend the forecasting process to allow for NEGATIVE forecasts! Right now
				the macro %ComputeForecast sets a lower bound of 0 to the generated forecasts (because
				this was a requiremente of the application for which this macro was developed).
				Note that this issue came up when forecasting a DIFFERENCED series which had ZERO MEAN
				in the context of the problem sent by Koen Knapen at SAS in May-2012.

				- CHOICE OF MODEL: Allow the user to specify the following criteria about model
				selection:
					- The goodness of fit measure to use.
						I propose to allow Relative Error (Mean or Median), Absolute Error (Mean or Median)
					- Which sample to use the selection criterion of goodness of fit (whether on all
						the data, on TRAIN or on VALID). Note that if we choose to use ALL the data
						or the TRAIN sample for model selection, the forecast to evaluate is the
						1-STEP FORECAST on such samples (as they are based on the data used for
						developing the model, and this is what is done by Forecast Server).
...
sample				Nombre de la variable que tiene la informacion de la muestra a la que pertenece cada
					observacion. Las diferentes muestras son 3: 1-TRAIN, 2-VALID, 3-TEST, que se usan
					de la siguiente manera:
					- 1-TRAIN: Se usa para ajustar todos los modelos candidatos.
					- 2-VALID: Se usa para elegir el mejor modelo entre todos los candidatos, basado
					en el MAPE (Median Absolute Percent Error) que se obtiene sobre dicha muestra.
					- 3-TEST:  Se usa para calcular el MAPE final arrojado por el modelo elegido
					sobre una muestra nunca vista.
					Default: Sample
timeid				Nombre de la variable que tiene el identificador de tiempo.
					Default: Fecha
criterion			What selection criterion should be used for model selection.
					Either one of the following values:
					- MAE:		Mean Absolute Error (based on the raw residuals)
					- MDAE:		Median Absolute Error (based on the raw residuals)
					- MAPE:		Mean Absolute Percent Error (based on the relative errors)
					- MDAPE: 	Median Absolute Percent Error (based on the relative errors)
					Default: MAPE
...
allownegative		= 1 allows negative forecasts.
					= 0 lower bounds generated forecasts with 0.
					Default: 1
...
plot				= 1 para generar los graficos con el ajuste del modelo y el pronostico,
					...
					Default: 0
plotwhat:			What periods to plot when plotting the forecasts.
					Possible values are AUTO or ALL.
					Default: AUTO
...
IMPORTANT: The TIMEID variable must contain EQUALLY spaced values according to its UNFORMATTED values.
This implies for example that a time variable whose interval is the MONTH is NOT ACCEPTED, because the
interval given by the UNFORMATTED values are not equally spaced in such case! There is no treatment
done by this macro to conclude that a monthly-spaced variable is actually equally spaced.
If such a time ID variable should be used, the solution is to create an auxiliary time ID variable
with consecutive numbers.
If a monthly-spaced variable is used as time ID, there would be ERRORS towards the end of the log file
when the best model is supposed to be selected.
===================================================================================================
...
V3.01		16/09/2011	DM		- STATIONARITY TEST: Added the check whether the Rho statistic is
								significant in the Dickey-Fuller test in addition to checking whether
								the Tau statistic is significant. This is because I found that some
								cases the Prob(Rho) may be small even when the Prob(Tau) is not small
								in the test for a trend and I thought this could indicate that there
								is actually a trend.
								HOWEVER, note that in Wei, Chapter 6, page 105, it says that under
								doubt it is better to difference the series than not difference it.
								This would suggest that the test for differencing should be based
								only on one statistic (perhaps the original one I was using Tau).
								In order to change this back to the original, simply change the
								line:
									if . < probtau < 0.05 or . < probrho < 0.05 then do;
								with:
									if . < probtau < 0.05 then do;
								(2012/05/30: I reverted to the original check ONLY on probtau after
								analyzing a series provided by Koen Knapen on 29-May-2012 where it
								is clear from the plot that the analyzed series (WE_Corrugated_Shipment)
								should be differenced and checking also on probrho suggests a linear
								trend but this does not make sense)
V3.02		17/10/2011	DM		- PLOT FORECASTS: Added parameter PLOTWHAT in order to allow the user
								to see ALL the series when plotting the forecasts.
								Possible values are AUTO and ALL, and the default is AUTO, where
								the starting date for plotting purposes is computed based on 
								the value of parameter PERIOD_MAX. See macro variable &plot_startdate
								in the macro below.
V3.03		31/05/2012	DM		- ARIMA ADJUSTMENT: Added a check that the the macro variables listed
								in the CROSSCORR= and INPUT= options are not emtpy before using those
								options. This solves an runtime error indicating that a variable list is
								expected for those options.
								- NEGATIVE FORECASTS: I relaxed the forecasts to be also negative.
								This implied doing the following changes:
									- I added a new input parameter called ALLOWNEGATIVE, which
									defaults to 1.
									- In %ComputeForecast, I added a new parameter called LOWERBOUND
									which specifies a lower bound value, defaulting to missing
									(=> no lower bound is used)
								- MODEL SELECTION CRITERION: Added the possibility for the user to
...
%MACRO ComputeForecast(target=, trend=0, logarithm=0, round=0, lowerbound=.);
	...
	%if &logarithm %then %do;
		...
		L95_log = L95;
		U95_log = U95;
		%*** DM-2012/05/31-START: Changed the zero lower bound with a flexible lower bound value;
		forecast = max(&lowerbound, 10**forecast_log - 1);
		L95 = max(&lowerbound, 10**L95_log - 1);
		U95 = max(&lowerbound, 10**U95_log - 1);
		%*** DM-2012/05/31-END;
	%end;
	%else %do;
		%*** DM-2012/05/31-START: Changed the zero lower bound with a flexible lower bound value;
		forecast = max(&lowerbound, forecast);
		L95 = max(&lowerbound, L95);
		U95 = max(&lowerbound, U95);
		%*** DM-2012/05/31-END;
	%end;
	%if &round %then %do;
	...
	%*** DM-2012/05/31-START: Added the computation of the residual and absolute residual in order to be able to use it
	%*** as another model selection criterion (note that we need to recompute the residual here even though the RESIDUAL
	%*** column is already in the FORECAST dataset generated by PROC ARIMA, because there may have been a lower bound
	%*** affecting the value of the FORECAST variable in this macro (depending on the LOWERBOUND parameter value);
	res = forecast - &target;
	res_abs = abs(res);
	%*** DM-2012/05/31-END;
	%*** DM-2012/05/31-START: Since negative forecasts are now allowed, changed > 0 with ~= 0 and added the ABS function;
	if &target ~= 0 then
		error = (forecast - &target) / abs(&target);
	else if forecast ~= 0 then
		error = (forecast - &target) / abs(forecast);
	else
		error = .;
	%*** DM-2012/05/31-END;
	error_abs = abs(error);
...
%local lead_valid;
%local lead_test;
%local train_startdate;		%* DM-2011/10/17: Created this local macro variable to allow plotting all the series when plotting the forecast;
...
data _NULL_;
	...
	%* DM-2011/10/17-START: Store the date where the TRAINING sample starts in case the user requests to see all the series plotted;
	if &sample = "1-TRAIN" then do;
		call symput ('train_startdate', trim(left(put(&timeid, date9.))));
	end;
	%* DM-2011/10/17-END;
...
run;
%* DM-2011/10/17-START: Check wether parameter plotwhat requests to plot all the series, in which case I update the value of
%* plot_startdate to the first available date in the TRAINING sample;
%if %sysfunc(upcase(&plotwhat)) = ALL %then
	%let plot_startdate = &train_startdate;
%* DM-2011/10/17-END;
...
%*** DM-2012/05/31-START: ALLOWNEGATIVE parameter indicating whether forecasts are lower bounded by 0 or not;
%* ALLOWNEGATIVE;
%let lowerbound = .;
...
/*------------------------------------- Initial Settings ------------------------------------*/
	...
	En lo que sigue los LAGS indican lags the la variable Y que se consideran como potencialmente
	generadores de una serie no estacionaria (ej. si el proceso Y(t) = Y(t-4) + e(t) entonces
	la serie es no estacionaria con un LAG=4).
	...
	*/
	...
	%if %sysfunc(exist(tempdata.com_&execID._dftest)) %then %do;
		...
		data _NULL_;
				...
				select (typen);
					when (1)
						do;	%* STATIONARY WITH NO INTERCEPT;
							...
							call symput ('probrho', trim(left(ProbRho)));
			...
			if lastobs then do;	%* DIFFERENCE SERIES (Solo llega aca si no entrO nunca en el IF . < PROBTAU < ... OR IF . < PROBRHO < ... de arriba);
				call symput ('stationary', trim(left(0)));
				call symput ('trend', trim(left(0)));
				call symput ('intercept', trim(left(0)));
				call symput ('type', trim(left(type)));
	...
	%put %quote(        P-valor (Rho)=&probrho);
	%put %quote(        P-valor (Tau)=&probtau);
	...
	data tempdata.com_&execID._stationary;
		...
		length stat_type $15;
		stat_type = "&type";
		stat_pvalue_rho = &probrho;
		stat_pvalue_tau = &probtau;
		stat_lags = &lags;
	run;
	...
	ods listing exclude all;
	proc arima data=tempdata.com_&execID._data2fit;
			identify var=&y2fit(&diffy) esacf p=(0:&pmax) q=(0:&qmax)
				/* DM-2012/05/31-START: Check if there are any INPUT variables for the cross-correlation analysis */
				%if %quote(&var_diff) ~= %then %do;
					crosscorr=(&var_diff)
				%end;
				/* DM-2012/05/31-END */
				;
	ods output 	esacf=tempdata.com_&execID._esacf
		...
		%** Note that the p and q orders to try are automatically defined by SAS and they are based
		%** on the p-values of the estimated ESACF values. The cut-off value for considering an ESACF
		%** value to be (in)significant is defined by the ALPHA option of the IDENTIFY statement
		%** which defaults to 0.05.
		%** (for more information on this criterion, see Wei, pag. 129);
	run;
	ods listing;
	/*--------------------------------------- ESACF -----------------------------------------*/
	...
	%do i = 1 %to &nro_esacf;
		...
		proc arima data=&data4arima;
			identify var=&y2fit(&diffy) 
				/* DM-2012/05/31-START: Check if there are any INPUT variables for the cross-correlation analysis */
				%if %quote(&var_diff &dum_add_diff &dum_shift_diff) ~= %then %do;
					crosscorr=(&var_diff &dum_add_diff &dum_shift_diff)
				%end;
				/* DM-2012/05/31-END */
				noprint;
			estimate &noint p=&ar_lags q=&ma_lags
				/* DM-2012/05/31-START: Check if there are any INPUT variables for the ARIMA model */
				%if %quote(&var &dum_add &dum_shift) ~= %then %do;
					input=(&var &dum_add &dum_shift)
				%end;
				/* DM-2012/05/31-END */
				method=ML plot printall;
		...
		%if &nobs > 0 %then %do;
			...
			data tempdata.com_&execID._forecast;
				...
				by &timeid;
				if in1;
				%*** DM-2012/05/31-START: Added the LOWERBOUND parameter;
				%ComputeForecast(target=&target, trend=&trend, logarithm=0, round=&round, lowerbound=&lowerbound);
				%*** DM-2012/05/31-END;
			run;

			%* Compute the aggregated error measures for VALID;
			proc means data=tempdata.com_&execID._forecast mean median stddev noprint;
				class &sample;
				%*** DM-2012/05/31-START: Added the RES and RES_ABS variables in the statistic computation as they are now computed in the ComputeForecast macro;
				var res 	res_abs
					error 	error_abs;
				%*** DM-2012/05/31-END;
	...
	%if %sysfunc(exist(tempdata.com_&execID._mapes)) %then %do;
		proc means data=tempdata.com_&execID._mapes noprint;
			...
			id &sample;		%*** DM-2012/05/12: Added this ID statement in order to easily indicate the inclusion of the SAMPLE variable in the output dataset;
			var res_abs_mean 	res_abs_median
				error_abs_mean 	error_abs_median;
			output out=tempdata.com_&execID._mape_pqmin(drop=_TYPE_ rename=(_FREQ_=n))
				/* DM-2012/05/31-START: Changed the IDGROUP clause with the MINID because I am now including three other variables
				   --besides ERROR_ABS_MEDIAN-- where the minimum is found. I was not able to figure out how to include several
				   variables for this calculation and in any case the MINID clause suffices */
				   minid(res_abs_mean(p q) 	 res_abs_median(p q)
						 error_abs_mean(p q) error_abs_median(p q)) = 	p_res_mean q_res_mean p_res_median q_res_median
						 												p_error_mean q_error_mean p_error_median q_error_median	
				   min=;
/*				   idgroup(min(error_abs_median) out(error_abs_median error_abs_mean &sample p q)=);
		...
		data tempdata.com_&execID._mape_pqmin;
			%*** DM-2012/05/31-START: Added new error measures: MAE, MDAE, and MAPE, renamed the original MAPE to MDAPE
			%*** and added the possibility for the user to specify which error measure to use for model selection;
			keep &sample n p q MAE MDAE MAPE MDAPE;
			format &sample n p q MAE MDAE MAPE MDAPE;
			set tempdata.com_&execID._mape_pqmin
			%if %quote(%upcase(&criterion)) = MAE %then %do;
				(rename=(p_res_mean=p q_res_mean=q))
			%end;
			%else %if %quote(%upcase(&criterion)) = MDAE %then %do;
				(rename=(p_res_median=p q_res_median=q))
			%end;
			%else %if %quote(%upcase(&criterion)) = MAPE %then %do;
				(rename=(p_error_mean=p q_error_mean=q))
			%end;
			%else %if %quote(%upcase(&criterion)) = MDAPE or %quote(&criterion) = %then %do;
				(rename=(p_error_median=p q_error_median=q))
			%end;
			;
			format MAPE MDAPE percent7.1;
			MAE = res_abs_mean;
			MDAE = res_abs_median;
			MAPE = error_abs_mean;
			MDAPE = error_abs_median;
	...
	%if ~&error_arima %then %do;
		data tempdata.com_&execID._mape_pqmin;
			...
			p = .;
			q = .;
			%*** DM-2012/05/31-START: Added new error measures: MAE, MDAE, and MAPE and renamed the original MAPE to MDAPE
			%*** (because originally I was calling MAPE to the MEDIAN absolute percent error);
			MAE = .;
			MDAE = .;
			MAPE = .;
			MDAPE = .;
		...
		proc arima data=tempdata.com_&execID._data4arima;
			identify var=&y2fit(&diffy)
				...
				/* DM-2012/05/31-END */
				;
			estimate &noint p=&ar_lags_best q=&ma_lags_best
				/* DM-2012/05/31-START: Check if there are any INPUT variables for the ARIMA model */
				%if %quote(&var &dum_add &dum_shift) ~= %then %do;
					input=(&var &dum_add &dum_shift)
				%end;
