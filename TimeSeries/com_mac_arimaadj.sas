...
Descripcion:	Macro para ajuste de un modelo ARIMA considerando aspectos de:
				...
				- Analisis de seasonality (periodicidades presentes en la serie)
				- Ajuste de varios modelos ARIMA sugeridos por el metodo ESACF
...
===================================================================================================
data				Dataset con la serie de tiempo a ajustar.
target				Nombre de la variable que contiene la serie de tiempo a ajustar.
y					Nombre de la variable auxiliar de la variable a ajustar que tiene missing
					en las muestras distintas a TRAIN.
varclass			Lista de variables CATEGORICAS a usar como predictoras de TARGET.
varnum				Lista de variables CONTINUAS a usar como predictoras de TARGET.
outforecast			Nombre del dataset de salida con el pronostico para la muestra TEST.
outforecastall		Nombre del dataset de salida con el pronostico para las muestras TRAIN, VALID y TEST.
outinfo				Nombre del dataset de salida con la informacion resumida sobre el modelo ajustado.
outmape				Nombre del dataset de salida con el MAPE y otros errores del modelo ajustado,
					sobre TRAIN y VALID.
outmapes			Nombre del dataset de salida con los MAPEs y otros errores de los distintos modelos
					ARIMA intentados segun la propuesta del metodo ESACF, sobre TRAIN y VALID.
outoutliers			Nombre del dataset de salida con la informacion de la deteccion de outliers en el
					modelo ajustado.
outparams			Nombre del dataset de salida con los parametros estimados del modelo ajustado.
outsummary			Nombre del dataset de salida con la informacion sobre la convergencia del modelo ajustado.
sample				Nombre de la variable que tiene la informacion de la muestra a la que pertenece cada
					observacion.
					Default: Sample
timeid				Nombre de la variable que tiene el identificador de tiempo.
					Default: Fecha
lead				Numero de periodos a pronosticar en TEST.
					Default: 1
interval			Intervalo de la variable TIMEID que separan los distintos periodos de medicion.
					Default: 1
pmax				Numero maximo del orden P a considerar en la identificacion ESACF de los posibles
					modelos ARIMA(p,q).
					Default: 4
qmax				Numero maximo del orden Q a considerar en la identificacion ESACF de los posibles
					modelos ARIMA(p,q).
					Default: 4
period				Periodo de la variable TIMEID a considerar en el ajuste de todos los modelos ARIMA.
period_min			Minimo valor de periodo como para considerar aceptable el periodo detectado en el 
					analisis de seasonality.
					Default: 5
period_max			Maximo valor de periodo como para considerar aceptable el periodo detectado en el
					analisis de seasonality.
					Default: 53
outliers_maxnum		Numero maximo de outliers a detectar (el numero maximo de outliers a detectar es
					el MINIMO entre este valor y el dado por el porcentaje de los datos pasados en
					OUTLIERS_MAXPCT)
					Default: 5
outliers_maxpct		Porcentaje maximo de outliers a detectar (el numero maximo de outliers a detectar es
					el MINIMO entre el numero representado por este porcentaje y el numero pasado en
					OUTLIERS_MAXNUM).
					Default: 10
outlierscondition	Condicion WHERE a aplicar en el proceso de deteccion de outliers.
					Ej: where '01dec2005'd <= Fecha <= '30aug2006'd
round				= 1 si se desea redondear el valor pronosticado en FORECAST al entero mas cercano.
					= 0 en caso contrario.
					Default: 0
id					Lista de variables ID que se arrastran a lo largo del proceso de ajuste.
case				Texto a mostrar en el log con informacion sobre el caso que se esta ajustando.
					Ej. Caso=3 Cliente=PRGAME Canal=13
plot				= 1 para generar los graficos con el ajuste del modelo y el pronostico,
					= 0 para no generarlos.
					Default: 0
error				Nombre de la macro variable que se genera al finalizar la macro
					donde se guarda un indicador de si ocurrio un error en el ajuste del 
					modelo ARIMA y en la generacion del forecast.
					Dicha macro variable debe existir antes de invocar a la macro desde el
					mundo exterior o debe estar definida como local si la macro es invocada 
					desde otra macro.
...
SUPUESTOS
===================================================================================================
1.- El dataset de entrada indicado en DATA tiene una variable que cuyo nombre se indica en el parametro
SAMPLE que indica a que muestra pertenece la observacion: TRAIN, VALID o TEST.
Los valores tomados por dicha variable son (en mayusculas): 1-TRAIN, 2-VALID, 3-TEST.
2.- La variable indicada en el parametro Y es missing en VALID y en TEST.
3.- No hay huecos en los datos de la variable indicada en el parametro TIMEID.
===================================================================================================
...
V3			15/09/06	DM		- Se agrego' el parametro OUTLIERSCONDITION para que la deteccion
								...
								- Se agrego' un proceso de deteccion de outliers como el que se
								hace para las distintas combinaciones de P y Q en el ajuste del
								mejor modelo para que se detecten outliers en fechas recientes al
...
/*------------------------------------- Initial Settings ------------------------------------*/
	...
	*/
	...
	%* Variable que indica si ocurre un error en el proceso de ajuste ARIMA (tras todas las iteraciones
	%* sobre los valores de P y Q);
	%let error_arima = 0;
	...
	%do i = 1 %to &nro_esacf;
		...
		%*** ARIMA OUTLIERS;
		%* NOTAR:
		%* - Aqui se pasa todo lo que sean periodicidades a diferenciacion de la variable a ajustar
		%* para evitar las inestabilidades numericas en la deteccion de outliers por altos ordenes de p y q
		%* - No se incluyen las variables predictoras en la deteccion para evitar posibles problemas generados
		%* por la definicion de un proyecto de alta complejidad que impacte en el proceso de deteccion de
		%* outliers;
		ods listing exclude all;
		proc arima data=tempdata.com_&execID._data2fit;
			&outlierscondition;
			identify var=&y2fit(&diffy %if %quote(&period) ~= %then %do; , &period %end; %if %quote(&accepted) = SI %then %do; , &periodf %end;) /*crosscorr=(&var_diff)*/ noprint;
	...
	%if ~&error_arima %then %do;
		...
		/*------------------------------------- OUTLIERS ------------------------------------*/
		%* Datasets creados en TEMPDATA: COM_OUTLIERS_BEST;
		...
		%* Datasets creados en TEMPDATA: COM_OUTLIERS;
		%put 6a.- DETECCION DE OUTLIERS PARA EL MODELO ELEGIDO;

		%*** ARIMA OUTLIERS;
		%* Borro dataset con los outliers detectados en el modelo final;
		proc datasets library=tempdata nolist;
			delete com_&execID._outliers_best;
		quit;
		...
		proc arima data=tempdata.com_&execID._data4arima;
			&outlierscondition;
			identify var=&y2fit(&diffy %if %quote(&period) ~= %then %do; , &period %end; %if %quote(&accepted) = SI %then %do; , &periodf %end;) /*crosscorr=(&var_diff)*/ noprint;
			estimate &noint p=&p_best q=&q_best /*input=(&var)*/ method=ML plot printall; run;
			outlier alpha=0.01 type=(additive shift) id=&timeid sigma=robust maxnum=&outliers_maxnum maxpct=&outliers_maxpct;
		ods output OutlierDetails=tempdata.com_&execID._outliers_best;
		run;
		ods listing;

		%let nout_add = 0;
		%let nout_shift = 0;
		%if %sysfunc(exist(tempdata.com_&execID._outliers_best)) %then %do;
			%** Notar que siempre se genera el dataset OUTLIERS a menos que el ESTIMATE de error;
			%** Si no se detectaron outliers el dataset OUTLIERS existe pero no tiene observaciones y
			%** por eso ahora me fijo si el nro. de obs en el dataset es > 0;
			%let nobs = 0;
			%callmacro(getnobs, tempdata.com_&execID._outliers_best return=1, nobs);
			%if &nobs > 0 %then %do;
				proc sort data=tempdata.com_&execID._outliers_best;
					by type time;
				run;
				...
				data tempdata.com_&execID._outliers_best;
					set tempdata.com_&execID._outliers_best end=lastobs;
					nout_add + (upcase(type) = "ADDITIVE");
					nout_shift + (upcase(type) = "SHIFT");
					if lastobs then do;
						call symput ('nout_add', trim(left(nout_add)));
		...
		%put 6b.- AJUSTE DEL MODELO ELEGIDO;
		...
		%* Reajusto el modelo ARIMA y calculo el 1-step ahead forecast para TEST reajustando los parametros usando TRAIN+VALID;
		ods listing exclude all;
		proc arima data=tempdata.com_&execID._data4arima;
