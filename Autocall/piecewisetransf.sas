...
Created: 	23-Nov-2004
Modified: 	04-Aug-2016 (previous: 16-Jun-2016)
...
%PiecewiseTransf(
	...
	dropallequal=0,	*** Whether to drop the created dummy variables that have the same values for all observations.
...
- dropallequal: Whether to drop from the output dataset the created dummy variables that have the same values
				for all observations.
				This is useful when following the execution of the macro, we use the global macro variables
				generated by the macro (_dummylist_ and _pwlist_) to fit a regression model where
				we do not want to include variables having all the same value since it generates
...
%MACRO PiecewiseTransf(
		...
		outformat=,
		log=1,
		help=0) / des="Piecewise transformation of continuous variables with specified cuts";
...
%MACRO ShowMacroCall;
	...
	%put dropallequal= , %quote(   *** Whether to drop the created dummy variables that have the same values for all observations.);
...
%if &log %then %do;
	...
	%put PIECEWISETRANSF: - dropallequal = %quote( &dropallequal);
...
data &out;
	...
	%do i = 1 %to &nro_vars;
		...
		%* Set the DROP&i* variables to 0 when DROPALLEQUAL = 0 because this indicates that
		%* we want to keep all variables created for variable I;
		%if ~&dropallequal %then %do;
			%let drop&i.0 = 0;
			%do j = 1 %to %eval(&nro_cutvalues+1);
				%let drop&i&j = 0;
...
*/
			...
			%* Note that in that case we still keep the variable in the output because to drop it we should do another data step!
...
%if %quote(&outformat) ~= %then %do;
	%if &cutsAreNumbers %then %do;
		...
		data _PT_cuts_;
			...
			array avars(*) $ var1-var&nro_vars (%quote(")%MakeList(&var, sep=%quote(" "))%quote("));
