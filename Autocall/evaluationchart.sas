...
%EvaluationChart(
	...
	odspath=, 						*** Unquoted name of the path where the ODS file should be saved
	odsfile=,						*** Unquoted name of the ODS file where the Evaluation Chart should be saved
	odsfiletype=pdf,				*** Type of the file specified in ODSFILE
	imagerootname=EvaluationChart,	*** Root name to be used for the image file name generated by ODS graphics
...
- odspath:			Unquoted name of the path where the Evaluation Chart should be saved.
					default: current working directory

- odsfile:			Unquoted name of the file where the Evaluation Chart should be saved in the
					format given by the ODSFILETYPE= parameter.
					If the odsfiletype is HTML, each file is saved in separate files
					with names defined by the IMAGEROOTNAME parameter as indicated below.
					default: (empty)
...
- imagerootname:	Root name to be used for the image file name generated by ODS graphics.
					The actual file name is <imageroot>.png, with possibly some number at
					the end to distinguish the files to avoid override if several plots are
					generated (e.g. in the case of BY variables with OVERLAY=0).
					Ex: EvaluationChart1.png
					default: EvaluationChart
...
%MACRO EvaluationChart(	data,
						...
						odsfile=,
						odsfiletype=pdf,
						imagerootname=EvaluationChart,

						libformat=,
						print=1,
						log=1,
						help=0) / des="Plots Evaluation Charts of a scoring model";
...
%MACRO ShowMacroCall;
	...
	%put odspath= ,	%quote(		         *** Unquoted name of the path where the ODS file should be saved.);
	%put odsfile= , %quote(              *** Unquoted name of the ODS file to store the Evaluation Chart);
	%put odsfiletype=pdf , %quote(       *** Type of the file specified in ODSFILE);
	%put imagerootname=EvaluationChart , *** Root name to be used for the image file name generated by ODS graphics.);
...
%if &log %then %do;
	...
	%put EVALUATIONCHART: - odspath = %quote(      &odspath);
	%put EVALUATIONCHART: - outfile = %quote(      &odsfile);
	%put EVALUATIONCHART: - outfiletype = %quote(  &odsfiletype);
	%put EVALUATIONCHART: - imagerootname = %quote(&imagerootname);
...
%*** BY=;
%* If BY= is not empty, separate the input dataset in as many data sets as the number of BY-variable values;
%if %quote(&by) ~= %then %do;
	%if &nro_data > 1 %then %do;
	...
	%else %do;
		...
		%do i = 1 %to &nbygroups;
			data _EC_data&i;
				merge 	&_data_
						_EC_freq_by_(in=in2 firstobs=&i obs=&i);
				by &by;
				if in2;
			run;
			%* Read the values of the BY variables to be shown in the MODEL column of the output 
			%* datasets and in the legend of the graph;
			%* NOTE: As of 29-Nov-2018 blank spaces are allowed in the VALUES of the by variables
			%* without generating an error message when calling the %CreateInteraction macro below
			%* This is achieved by replacing all blank values in a BY variable value with a hyphen;
			data _NULL_;
				set _EC_freq_by_(firstobs=&i obs=&i);
				%* NOTE: The below call to %MakeList makes it possible for BY-variable values to
				%* have blank spaces in it. Note that the use of LEFT() removes any blank spaces
				%* generated on the left of NUMERIC BY-variable values (which are converted to string
				%* by the TRIM() function. And the TRIM() function removes any leading or trailing blank
				%* from CHARACTER BY-variable values;
				call symput('byi', %MakeList(&by, 	prefix=%str( tranwrd%(trim%(left%( ), 
													suffix=%str( %)%), ' ', '-'%) ), 
													sep=%str(||' '||)) );
			run;
...
%if &plot %then %do;
	...
	ods graphics / IMAGENAME="&imagerootname" MAXLEGENDAREA=50;
	%ODSOutputOpen(&odsfile, odspath=&odspath, odsfiletype=&odsfiletype, macro=EVALUATIONCHART);
	...
	%ODSOutputClose(&odsfile, odsfiletype=&odsfiletype, macro=EVALUATIONCHART, log=&log);
