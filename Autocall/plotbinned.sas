...
Created: 		13-Aug-2015
Modified: 		29-Nov-2018 (previous: 26-Jul-2018, 16-Jul-2018, 15-Feb-2016, 12-Nov-2015)
SAS Version:	9.4
...
%MACRO PlotBinned(
		...
		log=1,
		help=0) / des="Makes scatter or bubble plots of a target variable vs. binned continuous variables";
...
%ExecTimeStart;
%if &plot %then %do;
	%ODSOutputOpen(&odsfile, odspath=&odspath, odsfiletype=&odsfiletype, macro=PLOTBINNED, log=&log);
%end;
...
%*				keep &by var label index &vari value &stat nobs &target fit_loess fit_loess_low fit_loess_upp;
	...
	%if &plot %then %do;
		...
		ods graphics / imagename="&datetime-&imagerootname-&target-%GetZeroPaddedNumber(&i, &nro_vars)-&vari" reset=index;	
			%** The above IMAGENAME option in ODS GRAPHICS defines the root of the files
			%** that are generated by all the with the name specified in the IMAGENAME= option;
