...
Created: 	27-Feb-2003
Modified: 	15-Feb-2016 (previuos: 12-Nov-2015)
...
This macro differs from th %Categorize macro in that it extends the capabilities of PROC RANK,
as this procedure cannot handle the second option for categorization listed above
...
EXAMPLES:
1.- %CategorizePercentiles(test , out=test_cat , var=x z , both=1 , groups=100);
...
2.- %CategorizePercentiles(test , var=x z , suffix=_c , groups=4);
...
3.- %CategorizePercentiles(test , var=x z , value=median , suffix=_c , both=0, percentiles=25 50 75 100);
...
4.- %CategorizePercentiles(test, var=x z, groupsize=10);
...
5.- %CategorizePercentiles(test, var=verylongvariablename longvarname, both=0, varvalue=verylongvariablename);
...
%MACRO CategorizePercentiles(
					...
					log=1,
					help=0) / des="Categorizes continuous variables based on percentile values";

/*----- Macro to display usage -----*/
%MACRO ShowMacroCall;
	%put CATEGORIZEPERCENTILES: The macro call is as follows:;
	%put %nrstr(%CategorizePercentiles%();
...
%if &help %then %do;
	%ShowMacroCall;
%end;
%else %if ~%CheckInputParameters(data=&data , var=&var , macro=CATEGORIZEPERCENTILES) %then %do;
	%ShowMacroCall;
%end;
%else %do;
/************************************* MACRO STARTS ******************************************/
...
%if &log %then %do;
	%put;
	%put CATEGORIZEPERCENTILES: Macro starts;
	%put;
	%put CATEGORIZEPERCENTILES: Input parameters:;
	%put CATEGORIZEPERCENTILES: - Input dataset = %quote(&data);
	%put CATEGORIZEPERCENTILES: - var = %quote(          &var);
	%put CATEGORIZEPERCENTILES: - by = %quote(           &by);
	%put CATEGORIZEPERCENTILES: - out = %quote(          &out);
	%put CATEGORIZEPERCENTILES: - condition = %quote(    &condition);
	%put CATEGORIZEPERCENTILES: - alltogether = %quote(  &alltogether);
	%put CATEGORIZEPERCENTILES: - varcat = %quote(       &varcat);
	%put CATEGORIZEPERCENTILES: - varvalue = %quote(     &varvalue);
	%put CATEGORIZEPERCENTILES: - suffix = %quote(       &suffix);
	%put CATEGORIZEPERCENTILES: - value = %quote(        &value);
	%put CATEGORIZEPERCENTILES: - both = %quote(         &both);
	%put CATEGORIZEPERCENTILES: - groupsize = %quote(    &groupsize);
	%put CATEGORIZEPERCENTILES: - groups = %quote(       &groups);
	%put CATEGORIZEPERCENTILES: - percentiles = %quote(  &percentiles);
	%put CATEGORIZEPERCENTILES: - descending = %quote(   &descending);
	%put CATEGORIZEPERCENTILES: - log = %quote(          &log);
	%put;
%end;
...
%do i = 1 %to &nro_vars;
	...
	%if %length(&_varcat_) > 32 or
		...
		%let error = 1;
		%put CATEGORIZEPERCENTILES: ERROR - For var = %upcase(&_var_), at least one name to be used for the categorized variables;
		%put CATEGORIZEPERCENTILES: is longer then 32 characters and this exceeds the maximum SAS length.;
		%put CATEGORIZEPERCENTILES: Consider using a shorter SUFFIX or the VARCAT= parameter in order to;
		%put CATEGORIZEPERCENTILES: specify new variable names for the categorized variables.;
		%put CATEGORIZEPERCENTILES: The macro will stop executing.;
	%end;
%end;
...
%* one dataset (_cp_data_) with the analysis variables and another dataset
%* (_cp_data_rest_) with the other variables.
%* Note the following:
...
%if %quote(&by) ~= %then %do;
	proc sort data=&data out=_cp_data_;
		by &by;
	run;
	%let data = _cp_data_;
%end;
...
%* at the end;
data _cp_data_(keep=&by &var _cp_obs_) _cp_data_rest_(drop=&var);
	set &data end=lastobs;
	_cp_obs_ = _N_;
run;

%* Read the number of observations to be effectively used;
%Callmacro(getnobs, _cp_data_ return=1, nobs);

%if &groupsize = %then %do;
	%* Calculo de los percentiles para cada variable;
	%if &log %then %do;
		%put CATEGORIZEPERCENTILES: Nro. of approx. equal groups used to categorize each variable = &nro_percentiles;
		%put CATEGORIZEPERCENTILES: Average size of each group = %sysfunc(round(%sysevalf(&nobs/&nro_percentiles)));
		%put;
	%end;
...
%*	%let pctlpre = %MakeList(&var , prefix=_ , suffix=_);
	...
	%if %quote(&condition) = %then %do;
		proc univariate data=_cp_data_ noprint;
			%if %quote(&by) ~= %then %do;
			by &by;
			%end;
			var &var;
			output 	out=_cp_percentiles_ 
					pctlpre=&pctlpre 
					pctlpts=&percentiles
					pctlname=&pctlname;
...
%*			%let pctlname = %MakeListFromName( , length=&nro_percentiles , start=1 , step=1 , prefix=P , suffix=_);
			...
			proc univariate data=_cp_data_ noprint;
				...
				var &_var_;
				output 	out=_cp_percentiles_i_
						pctlpre=&pctlpre
						pctlpts=&percentiles
						pctlname=&pctlname;
			run;
			%if &i = 1 %then %do;
				data _cp_percentiles_;
					set _cp_percentiles_i_;
				run;
			%end;
			%else %do;
				data _cp_percentiles_;
					set _cp_percentiles_;
					set _cp_percentiles_i_;
				run;
			%end;
		%end;
	...
	%if %quote(&by) ~= %then %do;
	proc sql;
		create unique index %scan(&by, 1, ' ') on _cp_percentiles_ (&by);
...
%*		%let &_var_._p = %MakeList(&pctlname , prefix=_&_var_._);
	...
	data _cp_data_;
		...
		%if %quote(&by) ~= %then %do;
		set 	_cp_data_;
		set		_cp_data_rest_;		/* DM-2015/10/28: This dataset was eliminated because the first two datasets need to be SET but the last one needs to be MERGEd, but this is not possible (o.w. there is repeat of BY variables) */
		set 	_cp_percentiles_  key=&by / unique;
/*		by &by;*/
		...
		set _cp_data_;
		set _cp_data_rest_;
		if _N_ = 1 then set _cp_percentiles_;
		%end;
...
%else %do; %* %if groupsize = ;
	...
	%if &log %then %do;
		%put CATEGORIZEPERCENTILES: Nro. of approx. equal groups used to categorize each variable = &nro_groups;
		%put CATEGORIZEPERCENTILES: Approx. size of each group = &groupsize;
		%put;
	%end;
	%do i = 1 %to &nro_vars;
		%let _var_ = %scan(&var , &i , ' ');
		%* Sort by current variable to create the group IDs;
		proc sort data=_cp_data_;
		...
		data _cp_data_;
			...
			%end;
			if _cp_count_ >= &groupsize and _VAR&i._CAT_ < &nro_groups then do;
				_VAR&i._CAT_ = _VAR&i._CAT_ + 1;
				_cp_count_ = 1;		%* Notar que no se resetea a 0 (sino a 1), porque el registro
...
%if %quote(&value) ~= %then %do;
	%do i = 1 %to &nro_vars;
		...
		proc sort data=_cp_data_;
			by &by _VAR&i._CAT_;
		run;
		proc means data=_cp_data_ &value noprint;
			by &by _VAR&i._CAT_;
			var &_var_;
			output out=_cp_values_(drop=_TYPE_ _FREQ_)
		...
		%* in the input dataset (here represented by _cp_data_) because if that is the case only the first
		%* occurrence of the output variable will be updated by this merge because the _cp_values_ dataset
		%* has only ONE observation per category whereas _cp_data_ has many cases per category
		...
		data _cp_data_;
			merge 	_cp_data_(drop=	%if %quote(&_varcat_) ~= and %quote(&_varvalue_) = and &both %then %do; 	&_varcat_._&value  %end;
...
%* Add back the other variables present in the input dataset into _cp_data_;
%* Note that _cp_data_rest_ need NOT be sorted because that dataset was
%* not touched and thus keeps the original order given by _cp_obs_;
proc sort data=_cp_data_;
	by _cp_obs_;
run;
proc sort data=_cp_data_rest_;
	by _cp_obs_;
run;
%* Create output dataset or update input dataset (if OUT= is empty).
%* Here any options coming in the OUT= parameter are executed;
%if &log %then %do;
	%put CATEGORIZEPERCENTILES: Changes performed in output dataset %upcase(&out_name):;
	%put;
%end;
options dkrocond=nowarn;
data &out;
	format &var_order;		%* Original order of variables in the input dataset;
	merge _cp_data_rest_ _cp_data_;
	by _cp_obs_;
	...
	%do i = 1 %to &nro_vars;
		...
		%if %quote(&_varcat_) ~= %then %do;
			...
			%if %quote(&value) = or &both %then %do;
				%if &log %then %do;
					%if %ExistVar(_cp_data_rest_, &_varcat_, log=0) or %ExistVar(_cp_data_,  &_varcat_, log=0)  %then
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_varcat_) has been REPLACED with its integer-valued categorization.;
					%else
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_varcat_) has been created containing the integer-valued categorization for %upcase(&_var_).;
				%end;
				drop &_varcat_;
...
%*			%if &both %then %do;
			%if %quote(&value) = or %quote(&value) ~= and &both %then %do;
				%if &log %then %do;
					%if %ExistVar(_cp_data_rest_, &_var_&suffix, log=0) %then
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_var_&suffix) has been REPLACED with its integer-valued categorization.;
					%else
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_var_&suffix) has been created containing the integer-valued categorization for %upcase(&_var_).;
				%end;
				drop &_var_&suffix;
		...
		%if &log %then %do;
			%if %quote(&value) ~= %then %do;
				%if %quote(&_varvalue_) ~= %then %do;
					%if %ExistVar(_cp_data_rest_, &_varvalue_, log=0) or %ExistVar(_cp_data_, &_varvalue_, log=0) %then
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_varvalue_) has been REPLACED with its statistic-valued categorization.;
					%else
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_varvalue_) has been created containing the statistic-valued categorization for %upcase(&_var_).;
				%end;
				%else %if %quote(&_varcat_) ~= and &both %then %do;
					...
					%if %ExistVar(_cp_data_rest_, &_varcat_._&value, log=0) %then
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_varcat_._&value) has been REPLACED with its statistic-value categorization.;
					%else
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_varcat_._&value) has been created containing the statistic-valued categorization for %upcase(&_var_).;
				%end;
				%else %do;
					%if %ExistVar(_cp_data_rest_, &_var_&suffixValue, log=0) %then
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_var_&suffixValue) has been REPLACED with its statistic-valued categorization.;
					%else
						%put CATEGORIZEPERCENTILES: Variable %upcase(&_var_&suffixValue) has been created containing the statistic-valued categorization for %upcase(&_var_).;
				%end;
			%end;
...
%if &log %then %do;
	%put;
	%put CATEGORIZEPERCENTILES: Macro ends;
	%put;
%end;
