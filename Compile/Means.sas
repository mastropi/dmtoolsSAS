...
%Means(
	...
	stat=, 			*** Statistics to be computed on the variables
	...
	options=,		*** Additional PROC MEANS OUTPUT statement options
...
- options:		Options to be used in the OUTPUT statement of PROC MEANS after the
				'/' symbol.
...
%Means(
	A, 
	var=x y z w t s,
	stat=n mean min P1 P5 P10 P25 P50 P75 P90 P95 P99 max stddev cv,
	out=Distribution,
	transpose=1
);
*/
...
%MACRO ShowMacroCall;
	...
	%put options= ,	%quote(             *** Additional options for the OUTPUT statement.);
...
data _Means_data_(keep=&byvars &class &id &var &weight);
	set &data;
run;

%*** Converting &var into a blank-separated list of variables;
%let var = %GetVarList(_Means_data_ , var=&var, log=0);
...
%if %quote(&class) ~= %then %do;
	%let class = %GetVarList(_Means_data_ , var=&class, log=0);
...
%let idst =;
%if %quote(&id) ~= %then %do;
	%let id = %GetVarList(_Means_data_ , var=&id, log=0);
	%let idst = id &id;
%end;
...
%if %quote(&out) ~= %then %do;
	...
	%let outputst = %str(output out=&out_name(&out_options &droptypefreqst) &statst / &options);
%end;
%else
	%let noprint = 0;
...
%if &log %then
	%put MEANS: Dataset %upcase(_Means_data_) above refers to dataset %upcase(%scan(%quote(&data), 1, '(')).;
