...
# 15-Feb-2016
# Script that calls the a_RemoveStore.awk AWK program to each .sas file located in the -s or --sourcedir directory
# and generates the corresponding .sas output file in the -o or --outdir directory.
#
# EXAMPLE:
# bash a_RemoveStore.sh
...
OUTDIR="../CompileNoStore"				# Output directory with the processed file
...
# Process each of the SAS macro with a_RemoveStore.awk
let i=0
...
do
	...
	# Run AWK!
	if [ $TEST == 0 ]; then
		LC_ALL=C gawk -f a_RemoveStore.awk "$FILE" > "$OUTDIR/$OUTFILENAME"			# Note the use of LC_ALL=C, o.w. lines are truncated when an accented character is encountered (Ref: http://www.catonmat.net/blog/ten-awk-tips-tricks-and-pitfalls)
		
		echo -n ".......Checking that the number of lines are the same in input and output files..."
		...
		if [ $NLINES_IN -ne $NLINES_OUT ]; then
			echo
			echo "    WARNING: The number of lines in the output file name ("$NLINES_OUT") is not the same as in the input file name ("$NLINES_IN")"
		else
			echo OK
		fi
	fi
...
### Verification step
if [ $TEST == 0 ]; then
	# That all STORE keywords have been removed
	echo "Checking that the STORE keyword has been removed from the output files (nothing should appear now)..."
	find $OUTDIR -name "*.sas" -exec grep -i "/[ ]*store" {} \;
