/* MACRO %LiftChart --> Calls %EvaluationChart with specific parameter values
Version: 	2.01
...
DESCRIPTION:
This macro plots a lift chart for a scoring model. The target variable needs not be dichotomous.
...
NOTES:
This macro calls %EvaluationChart with option chart=LIFT.
See that macro for more information.
*/
&rsubmit;
%MACRO LiftChart(	data,
					target=y,
					score=p,
					by=,
					event=1,
					...
					print=1,
					log=1, 
					help=0) / store des="Plots Lift Charts for a scoring model";
%if &log %then
	%put LIFTCHART: Calling %nrstr(%EvaluationChart) with option chart=LIFT...;
%EvaluationChart(	&data,
					...
					chart=LIFT,					/* THIS PARAMETER IS FIXED */
					plot=&plot,
					overlay=&overlay,
					best=&best,
